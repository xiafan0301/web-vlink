...
export const drawBorder = {
  init (sid) {
    let nTarget = document.getElementById(sid);
    let startX, startY, offsetX, offsetY;
    // e.target.offsetLeft
    offsetX = Math.floor($(nTarget).offset().left);
    offsetY = Math.floor($(nTarget).offset().top);
    nTarget.onmousedown = function (e) {
      ...
      active_box.className = 'player_box';
      active_box.style.left = startX + 'px';
      active_box.style.top = startY + 'px';
      nTarget.appendChild(active_box);
      active_box = null;
    }
    // 鼠标移动
    nTarget.onmousemove = function (e) {
      // 更新 box 尺寸
      if(document.getElementById('active_box') !== null) {
        var ab = document.getElementById('active_box');
        ab.style.width = e.pageX - offsetX - startX + 'px';
        ab.style.height = e.pageY - offsetY - startY + 'px';
      }
    };
    // 鼠标抬起
